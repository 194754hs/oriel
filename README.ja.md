...
# 保存/開くの両方で28項目。未知のインターフェイスが本物へ落ちること、
# Show が本物のダイアログを出すことまで含む。
...
まだのもの: シムは Oriel のピッカーではなく本物のダイアログで応答する。コード署名なし。インストーラーなし。リリースバイナリなし。タグ付きリリースが出るまでは、公開されている挙動は暫定として扱ってください。
