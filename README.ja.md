...
>
> **UIは日本語のみです。** 文字列は全てソースに直書きされていて、多言語化の層がありません。上のスクリーンショットがビルドしたままの姿です。
...
まだのもの: シムは Oriel のピッカーではなく本物のダイアログで応答する。多言語化なし(UIは日本語のみ)。コード署名なし。インストーラーなし。リリースバイナリなし。タグ付きリリースが出るまでは、公開されている挙動は暫定として扱ってください。
