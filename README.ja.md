...
**[0.1.0 をダウンロード (Windows x64)](https://github.com/194754hs/oriel/releases/tag/v0.1.0)** — 展開して `oriel.exe` を実行。ファイラ側は何も登録しません。同梱のダイアログシムは `shim-control.ps1` を実行するまで不活性です（[先に読んでください](#ダイアログのシム)）。コード署名はありません。

![カラム表示の Oriel。リポジトリ直下・.github・ISSUE_TEMPLATE と、入るたびに右へカラムが開き、選択中ファイルの種類・サイズ・変更日を出すプレビューが続く。下部に辿ってきた経路がパンくずで出ている](docs/images/columns.ja.png)
...
![グリッド表示の Oriel。ファイルのサムネイルが格子状に並び、ツールバーに表示切り替えがある](docs/images/grid.ja.png)
